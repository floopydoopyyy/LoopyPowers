...
public class PowerCommand {
        ...
        for (ServerPlayerEntity p : targets) {
            ...
            if (power != null) {
                ...
                for (AbilityTypes type : AbilityTypes.values()) {
                    ...
                    sb.append("\n  ")
                            .append(type.color).append(abilityName).append("§7: ");
    ...
    private static int helpSelfPower(CommandContext<ServerCommandSource> ctx) {
        ...
        sendHelp(ctx.getSource(), type, power);
        return 1;
    }
    ...
    private static int helpNamedPower(CommandContext<ServerCommandSource> ctx) {
        ...
        sendHelp(ctx.getSource(), type, factory.get());
        return 1;
    }

    private static void sendHelp(ServerCommandSource src, String type, Power power) {
        Text msg;

        switch (type) {
            case "overview" -> msg = Text.literal(
                    "§b" + power.getName() + "§f\n" +
