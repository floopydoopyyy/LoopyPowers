...
public class SpaceRitual implements Ritual {
    ...
    private static final List<Supplier<Power>> SPACE_POWERS = List.of(
            CosmicPower::new, TeleportPower::new, DimensionalPower::new);
    ...
    private void tickStage1(ServerPlayerEntity sp, ServerWorld world, int t) {
        if (t == 1) {
            ...
            world.playSound(null, sp.getBlockPos(), SoundEvents.BLOCK_BEACON_AMBIENT, SoundCategory.PLAYERS, 0.7f, 0.35f);
        }
    ...
    private void tickStage2(ServerPlayerEntity sp, ServerWorld world, int t) {
        if (t == 1)  world.playSound(null, sp.getBlockPos(), ModSounds.DARKNESSTELEPORT2, SoundCategory.PLAYERS, 0.7f, 0.45f);
        if (t == 18) world.playSound(null, sp.getBlockPos(), SoundEvents.BLOCK_END_PORTAL_FRAME_FILL, SoundCategory.PLAYERS, 0.5f, 0.65f);
        if (t == 42) world.playSound(null, sp.getBlockPos(), SoundEvents.BLOCK_AMETHYST_CLUSTER_PLACE, SoundCategory.PLAYERS, 0.5f, 0.45f);
    }

    private void tickStage3(ServerPlayerEntity sp, ServerWorld world, int t) {
        if (t == 1) world.playSound(null, sp.getBlockPos(), SoundEvents.ENTITY_WARDEN_HEARTBEAT, SoundCategory.PLAYERS, 0.9f, 0.65f);
        double progress = (double) t / STAGE_3_TICKS;
        if (t % 12 == 0)
            world.playSound(null, sp.getBlockPos(), SoundEvents.ENTITY_WARDEN_HEARTBEAT,
                    SoundCategory.PLAYERS, (float)(0.5 + progress * 0.5), 0.65f + (float) progress * 0.45f);
        if (t == 32)
            world.playSound(null, sp.getBlockPos(), SoundEvents.ENTITY_WARDEN_SONIC_BOOM, SoundCategory.PLAYERS, 0.6f, 1.3f);
    }

    private void tickStage4(ServerPlayerEntity sp, ServerWorld world, int t) {
        if (t == 1) {
            world.playSound(null, sp.getBlockPos(), SoundEvents.ENTITY_WITHER_SPAWN, SoundCategory.PLAYERS, 0.9f, 0.55f);
            world.playSound(null, sp.getBlockPos(), SoundEvents.ENTITY_LIGHTNING_BOLT_THUNDER, SoundCategory.PLAYERS, 0.8f, 0.65f);
        }
        double progress = (double) t / STAGE_4_TICKS;
        if (progress < 0.80f)
            sp.addStatusEffect(new StatusEffectInstance(StatusEffects.LEVITATION, 10, 1, true, false, false));
        ...
        if (t % 4 == 0)
            world.playSound(null, sp.getBlockPos(), ModSounds.DARKNESSLOOP, SoundCategory.PLAYERS, 0.6f, 0.7f);
    }

    private void onComplete(ServerPlayerEntity sp, ServerWorld world) {
        Supplier<Power> factory = SPACE_POWERS.get(sp.getRandom().nextInt(SPACE_POWERS.size()));
        ...
        sp.extinguish();
        sp.heal(3.0f);
        world.playSound(null, sp.getBlockPos(), SoundEvents.UI_TOAST_CHALLENGE_COMPLETE, SoundCategory.PLAYERS, 1.0f, 1.1f);
